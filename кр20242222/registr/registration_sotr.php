...
<?php
        ...
        ?>
    <div class="registr">
        <h1 class="text233">Регистрация сотрудника отеля</h1>
            <form action="/admin/controllers/registration_sotr2.php" method="post" 
            ...
            <div class="mb-3">
                    <label for="obrazovanie" class="form-label fs-5">Образование<br></label>
                    <select class="vvod vvod23" name="obrazovanie">
                        <option value="Высшее">Высшее</option>
                        <option value="Среднее">Среднее</option>
                    </select>
            </div>
            <!-- <label for="id_zayavka" class="form-label fs-5">Выберите номер заявки<br></label>
            <select class="vvod vvod23" name="id_zayavka">
            <?php
            ...
            <label for="id_dolzhnost" class="form-label fs-5"><br>Должность<br></label>
            <select class="vvod vvod23" name="id_dolzhnost">
            <?php
            $sql= "SELECT id_dolzhnost, nazvanie FROM dolzhnost";
            $result = $connect->query($sql);
            $data = "";
            while($row = $result->fetch_assoc()){
                $data .=sprintf("<option value='%s'>%s</option>",$row['id_dolzhnost'],$row['nazvanie']);
            }
            echo $data;
            ?>
            </select>
